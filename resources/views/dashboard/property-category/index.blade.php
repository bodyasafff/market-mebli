    ...
    <div class="mainTable mdl-grid">
        ...
        <div class="mdl-cell mdl-cell--12-col mdl-cell--8-col-tablet mdl-cell--4-col-phone mdl-grid">
            <div class="mdl-cell mdl-cell--9-col mdl-cell--4-col-tablet mdl-cell--2-col-phone">
                <a href="{{route('dashboard.property-category.create') }}" class="mdl-button mdl-js-button mdl-button--raised mdl-button--accent" style="margin-top: 10px;background-color: #8bc34a">Нова категорія властивостей</a>
            </div>

            @include('widget.form.chosen-select-multiple', ['id' => 'property_categories', 'title' => 'Виберіть категорії продуктів', 'mdlCell' => [3, 4, 2],
                'options' =>  \App\Models\ProductCategory::select(['id','name_ua'])->get(),
                'optionName' => 'name_ua',
                'cssClass' => 'search-choise-block',
                'name' => 'property_categories-filter-select',
             ])
    ...
    <script>
        $(function () {
            ...
            dataTable = $('#data-table').DataTable({
                ajax: {
                    url: '{{ route('dashboard.property-category.indexJson') }}',
                    type: 'post'
                },
                order: [
                    [1, "desc"],
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        searchable: false,
                        orderable: false,
                        className: 'datatable-row-property_categories-filter mdl-cell--hide',
                    },
                    {
                        data: 'name_ua',
                        title: 'Назва',
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            return '<button class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect" onclick="location.href = \'/dashboard/property-category/' + row.id + '/edit\'">&nbsp;&nbsp;Редагувати&nbsp;&nbsp;</button>';
                        },
                    },
                    {
                        data: null,
                ...
                initComplete: function () {
                    ...
                    dataTablesFunc.selectFilter.initComplete(dataTable, 'property_categories');
                    defaultChosenInit();
                },
                processing: true,
                serverSide: true,
            ...
            dataTablesFunc.selectFilter.clickEventInit(dataTable, 'property_categories');
        })
        function deleteProductCategory(rowId) {
            if(confirm('Видалити?')){
                location.href = '/dashboard/property-category/'+rowId+'/destroy'
            }
        }
    </script>
