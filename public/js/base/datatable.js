...
var dataTablesFunc = {
    ...
    inputFilter: {
        ...
        initComplete: function (dataTable, field) {
            ...
            if(data.columns[dataTable.column('.datatable-row-'+field+'-filter').index()].search.value !== ''){
                $('#'+field+'-filter-input')
                    .val(data.columns[dataTable.column('.datatable-row-'+field+'-filter').index()].search.value)
    ...
    detailsRow: {
        init: function (dataTable, showDetailsRow) {
...
function deleteRowDataTable(e) {
    if (confirm("Delete the record?")) {
        ...
        $.ajax({
            url: base_repository.getBaseUrl()+data.id + '/destroy',
            method: "POST"
