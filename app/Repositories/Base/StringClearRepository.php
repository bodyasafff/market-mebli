...
{
    static function prepareQueryToRedisSearch($searchQuery, $or = false, $floatBegin = false, $floatEnd = false, $removeLastChar = false)
    {
                ...
                if(!empty($floatBegin) || !empty($floatEnd)){
                    foreach ($searchQueryArr as $i => $v){
                        ...
                        if(!is_numeric($v) && mb_strlen($v) > 2){
                            ...
                            }else if($removeLastChar && mb_strlen($v) > 3){
                                $r[] = mb_substr($v, 1);
                                $r[] = mb_substr($v, 0, -1);
                            }
        ...
        }else if(AppConf::NOT_USE_LATIN_AND_CYRILLIC){
            $searchQueryArr = explode(' ', $searchQuery);
            $searchQueryArr = array_values(array_unique(array_filter($searchQueryArr)));
    ...
    static function clearSpace($str, $removeTabs = true, $removeLineFeeds = true)
    {
        ...
        $str = StringClearRepository::clearDblSpace($str, $removeTabs, $removeLineFeeds);
    ...
    static function clearDblSpace($str, $removeTabs = true, $removeLineFeeds = true)
    {
